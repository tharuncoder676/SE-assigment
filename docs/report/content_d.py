...
from docbuild import bullets, code, h1, h2, link_para, note, para, table
...
def section17(doc):
    ...
    note(doc, "**We want to be explicit about what this is not.** The figures above are a "
              ...
              "submission is the one produced by the department's own tool, and Section 17.6 is "
              "left for it.")
    ...
    doc.add_page_break()
    h2(doc, "17.7  Attachment — institutional similarity report")
    para(doc, "Paste the similarity receipt or screenshot in the space below, then complete the "
              "record and signatures.", space_after=4)
    _placeholder(doc)
